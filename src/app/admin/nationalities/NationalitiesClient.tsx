...
export function NationalitiesClient({ rows }: { rows: Row[] }) {
  const t = useT();
  const { push } = useToast();
  ...
  function handleAdd() {
    ...
    startTransition(async () => {
      try {
        await createNationality(nameEn, nameAr);
        ...
        setError(e instanceof Error ? e.message : "Failed to add nationality.");
      }
    });
  }
  ...
  const columns: Column<Row>[] = [
    ...
    {
      key: "internCount",
      header: t("admin.nationalities.interns"),
      hideOnMobile: true,
      render: (r) => (
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <h1 className="text-[26px] sm:text-[30px] font-semibold">{t("admin.nationalities.title")}</h1>
        <Button onClick={() => setOpen(true)} className="sm:w-auto">
          {t("admin.nationalities.add")}
        </Button>
      </div>
      ...
      <Modal
        ...
        title={t("admin.nationalities.add")}
        footer={
          <>
